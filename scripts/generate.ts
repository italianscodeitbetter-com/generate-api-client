...
const API_CLIENT_AUGMENT_FILENAME = "apiClient.augment.ts";

/** Always overwritten — re-exports from the preserved augment file. */
function generateApiClientCustomStub(): string {
  return `// Auto-generated by api-client-generate. Edit apiClient.augment.ts for overrides (that file is never overwritten).

export { augmentApiClient } from "./apiClient.augment.js";
`;
}

/** Written only when apiClient.augment.ts is missing; then never overwritten. */
function generateApiClientAugmentScaffold(): string {
  return `// Overrides and bypasses for the generated API client. This file is not overwritten by the generator.

import type { GeneratedApiClient } from "./apiClient.js";

export function augmentApiClient(
  base: GeneratedApiClient,
): GeneratedApiClient {
  return base;
}
`;
...
async function main(): Promise<void> {
  ...
  const apiClientAugmentPath = join(outDir, API_CLIENT_AUGMENT_FILENAME);
  const augmentScaffold = generateApiClientAugmentScaffold();
  const apiClientAugmentExisted = existsSync(apiClientAugmentPath);

  if (!apiClientAugmentExisted) {
    let augmentInitialBody = augmentScaffold;
    if (existsSync(apiClientCustomPath)) {
      const prevCustom = readFileSync(apiClientCustomPath, "utf-8");
      const alreadyStub = prevCustom.includes("apiClient.augment.js");
      if (!alreadyStub && prevCustom.includes("export function augmentApiClient")) {
        augmentInitialBody = prevCustom;
      }
    }
    writeFileSync(apiClientAugmentPath, augmentInitialBody);
  }

  writeFileSync(apiClientCustomPath, generateApiClientCustomStub());
  ...
  console.log(`  - ${API_CLIENT_CUSTOM_FILENAME} (generated)`);
  console.log(
    `  - ${API_CLIENT_AUGMENT_FILENAME}${apiClientAugmentExisted ? " (left unchanged)" : " (created)"}`,
  );
